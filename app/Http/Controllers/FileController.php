...
class FileController extends Controller
{
    ...
    public function get_list(){
        ...
        $files = File::where('user_id',Auth::user()->id)->where('is_active', 1)->get();
    ...
    public function get_file($id){
        ...
        $file = File::where('user_id',Auth::user()->id)->where('is_active', 1)->where('id',$id)->first();
        if($file == NULL){
            return abort(404);
        }
    ...
    public function update(Request $request){
        if($request->submit == "update"){
            ...
            if($file != null){
                ...
                $name = $file->getClientOriginalName();
                $move = $file->move($destinationPath, $name);
                File::where('is_active',1)->where('id', $request->id)->update(['url'=>$url, 'name'=>$request->nama_file]);
